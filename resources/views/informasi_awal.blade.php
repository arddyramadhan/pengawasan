        ...
        <div class="card">
            <div class="card-header p-2">
                <ul class="nav nav-pills">
                    ...
                    <li class="nav-item"><a class="nav-link {{Request::segment(2)=='ditolak' ? 'active' : ''}}" href="/informasi_awal/tolak">Ditolak</a></li>
							...
							<div class="timeline" id="informasi" style="z-index: 1;">
								...
								<div>
									...
									<div class="timeline-item">
										<span class="time"><i class="fas fa-clock"></i>{{ $item->status }}</span>
										<h3 class="timeline-header"><a href="/informasi/{{ $item->id }}/show"> {{ $item->nama }}</a> <strong>|</strong> <span
											...
											@elseif($item->status == 'ditangani')
												...
												@else
												 <strong>|</strong> {{ $item->informasi_awal->user->name }}
												@endif
								...
								@endphp
									@foreach($data as $no => $item )
										...
										<div>
                                            ...
                                            {{-- @if ($item->status == 'diproses' && date('d-m-y', strtotime($item->created_at)) <= date('d-m-y', strtotime('+7 days', strtotime($item->created_at))))
                                            ...
                                            {{-- @endif --}}
												...
												<h3 class="timeline-header"><a href=""> {{ $item->informasi->nama }}</a>
													<strong>|</strong> Kode : <span class="text-danger text-bold">IA{{sprintf("%04d", $item->id) }}</span>
													@if($item->status == 'lengkapi')
														@if(Auth::user()->id == $item->id_user)
															<strong>|</strong> Dilengkapi : Saya
														@else
															 <strong>|</strong> Dilengkapi : {{ $item->user->name }} 
														@endif
													@elseif($item->informasi->status == 'ditangani')
														@if(Auth::user()->id == $item->id_user)
															<strong>|</strong> Ditanganni : Saya
														@else
															 <strong>|</strong> Ditanganni : {{ $item->user->name }} 
														@endif
        ...
        <div class="modal-content">
			...
			<form action="/informasi_awal/buatlangsung" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" value="{{date('Y-m-d\TH:i')}}" name="waktu_kejadian" class="form-control">
